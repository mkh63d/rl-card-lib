...
class HtmlReport:
    ...
    def _overview(self) -> str:
        ...
        for record in self.runs:
            ...
            rows.append([
                ...
                (f'<span>{_escape(metric_name)} '
                 f'<span class="chip">{_escape(span)}</span></span>')
    ...
    def _comparison_section(self, game: str) -> str:
        """One game's cross-agent comparison section.
        ...
        out = [f'<section id="game-{_slug(game)}">']
    ...
    def to_html(self) -> str:
        ...
        # Each game's comparison section immediately followed by that game's
        # own run-detail sections, so the body's reading order matches the
        # nav's nesting (game, then its agents, then the next game) instead
        # of grouping all games' graphs before any game's run details.
        runs_by_game: dict[str, list] = {}
        for record in self.runs:
            runs_by_game.setdefault(record.game, []).append(record)
        for game in self._games():
            body.append(self._comparison_section(game))
            for record in runs_by_game[game]:
